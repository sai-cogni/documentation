...
Phoenix's terminal integration lets you bring your terminal output directly into your conversations with Phoenix. Instead of copying and pasting error messages or command results, you can send them to Phoenix with a simple right-click in the terminal.

<Frame>
	<img
		src="https://storage.googleapis.com/phoenix_public_images/docs/assets/terminal-integration.png"
...
When you're working in the VSCode terminal and see output you want to discuss with Phoenix:

1. Right-click in the terminal
2. Select "Add to Phoenix" from the context menu
3. The terminal output is immediately sent to the Phoenix chat panel
...
Instead of switching context to the Phoenix chat panel and typing a description of the problem, you can send the terminal output directly to Phoenix with just a couple of clicks.

Alternatively, you can use the [`@terminal`](/features/at-mentions/terminal-mentions) mention to send the full terminal output to Phoenix.
...
When you use the right-click terminal integration, Phoenix:
...
This gives Phoenix the full context it needs to provide accurate help with terminal-related issues.

## Behind the Scenes

The terminal integration uses a clever technique to capture terminal output:

1. When you trigger the integration, Phoenix:
...
-   **Use terminal integration for error messages**: When you encounter an error in the terminal, sending it to Phoenix often results in faster resolution than trying to describe the error.
...
-   **Combine terminal outputs with file mentions**: After sending terminal output to Phoenix, you can enhance your question by mentioning relevant files using the @ mentions feature.
...
Next time you're staring at a cryptic error message in your terminal, try using Phoenix's terminal integration instead of copying and pasting. You'll get more accurate help because Phoenix can see the complete terminal context with proper formatting.
...
<Tip>
	**Quick Fix**: Most terminal issues can be resolved by switching to bash in the Phoenix settings and increasing the shell
