---
title: "MCP Overview"
description: "Learn about Model Context Protocol (MCP) servers, their capabilities, and how Phoenix can help build and use them. MCP standardizes how applications provide context to LLMs, acting like a USB-C port for AI applications."
---
...
<Frame>
	<img
		src="https://storage.googleapis.com/phoenix_public_images/docs/assets/mcp-diagram.png"
...
Phoenix does not come with any pre-installed MCP servers. You'll need to find and install them separately.
...
-   **Phoenix Marketplace:** Install one from Phoenix's [MCP Marketplace](/mcp/mcp-marketplace)
-   **Ask Phoenix:** You can ask Phoenix to help you find or create MCP servers
...
Phoenix simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct Phoenix in natural language to build an MCP server by describing its functionalities. Phoenix will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** Phoenix can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** Phoenix handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** Phoenix helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** Phoenix seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** Phoenix can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, Phoenix could use a GitHub server to get data and a Notion server to create a formatted report.
