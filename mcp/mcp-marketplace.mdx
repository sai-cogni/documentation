---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance Phoenix's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance Phoenix's capabilities. They enable Phoenix to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In Phoenix, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), Phoenix will guide you through:
...
-   Phoenix will show confirmation when installation is complete
-   Check the server status in Phoenix's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Phoenix will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Phoenix's system prompt
-   Simply ask Phoenix to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Phoenix in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   The server code is cloned/installed to `/Users/<username>/Documents/Phoenix/MCP/`
...
-   This file is located at: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/phoenix_mcp_settings.json`
-   Environment variables (like API keys) are securely stored
-   The server path is registered

### 3. Server Launch

-   Phoenix detects the configuration change
-   Phoenix launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with Phoenix

-   Your server's capabilities are added to Phoenix's system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   Phoenix can now use these capabilities when prompted by the user
...
1. Open the MCP settings file: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/phoenix_mcp_settings.json`
...
-   Try removing and reinstalling the server (remove from both the `phoenix_mcp_settings.json` file and the `/Users/<username>/Documents/Phoenix/MCP/` directory)
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Phoenix is seeing

## MCP Server Rules

Phoenix is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.phoenixrules` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making Phoenix's tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with Phoenix.
...
-   **Contextual Tool Selection**: Phoenix selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", Phoenix detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.phoenixrules` file or to your custom instructions to make Phoenix's MCP server selection more intuitive and context-aware.
