---
title: "AWS Bedrock"
description: "Learn how to set up AWS Bedrock with Phoenix using credentials authentication. This guide covers AWS environment setup, regional access verification, and secure integration with the Phoenix VS Code extension."
---
...
-   **Phoenix:** A VS Code extension that acts as a coding assistant by integrating with AI models—empowering developers to generate code, debug, and analyze data.
...
To ensure Phoenix can interact with AWS Bedrock, your IAM user or role needs specific permissions. While the `AmazonBedrockFullAccess` managed policy provides comprehensive access, for a more restricted and secure setup adhering to the principle of least privilege, the following minimal permissions are sufficient for Phoenix's core model invocation functionality:
...
2.  Use the JSON editor to add the following policy document:
    ...
    ```
3.  Name the policy (e.g., `PhoenixBedrockInvokeAccess`) and attach it to your IAM user or role.
...
-   **Model Listing in Phoenix:** The minimal permissions (`bedrock:InvokeModel`, `bedrock:InvokeModelWithResponseStream`) are sufficient for Phoenix to _use_ a model if you specify the model ID directly in Phoenix's settings. If you rely on Phoenix to dynamically list available Bedrock models, you might need additional permissions like `bedrock:ListFoundationModels`.
...
### Step 3: Configure the Phoenix VS Code Extension

#### 3.1 Install and Open Phoenix

1. **Install VS Code:**\
   Download from the [VS Code website](https://code.visualstudio.com/).
2. **Install the Phoenix Extension:**
    - Open VS Code.
    - Go to the Extensions Marketplace (`Ctrl+Shift+X` or `Cmd+Shift+X`).
    - Search for **Phoenix** and install it.

#### 3.2 Configure Phoenix Settings

1. **Open Phoenix Settings:**
...
By following these steps, your enterprise team can securely integrate AWS Bedrock with the Phoenix VS Code extension to accelerate development:
...
3. **Configure Phoenix in VS Code:** Install and set up Phoenix with your AWS credentials and choose an appropriate model.
...
_This guide will be updated as AWS Bedrock and Phoenix evolve. Always refer to the latest documentation and internal policies for up-to-date practices._
